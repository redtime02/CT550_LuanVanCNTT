...
const Reports = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3000/api/feedback/report"
      );
  ...
  const formattedData = data.map((item, index) => ({
    ...
    action: (
      <>
        ...
        {/* <button
          classNa) => showMome="ms-3 fs-3 text-danger bg-transparent border-0"
          onClick={(dal(enqState[i]._id)}
        >
          <AiFillDelete />
        </button> */}
      </>
  ...
  return (
    <div className="mt-4">
      <h3 className="mb-4">Danh sách báo cáo từ Người thu gom</h3>
      <div>
